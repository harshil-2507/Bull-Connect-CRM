...
export default function Leads() {
  ...
  const fetchLeads = async () => {
    try {
      ...
      const response = await fetch("https://bull-connect-crm.onrender.com/leads", {
        headers: {
  ...
  // Filtered leads based on selectedFilter
  const filteredLeads = leads.filter((lead) => lead.status === selectedFilter);
    ...
    <SafeAreaView className="flex-1 px-4 pt-4 bg-gray-50">
      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <Lead item={item} onAction={(lead) => setSelectedLead(lead)} />
          )}
          refreshControl={
            <RefreshControl
      ...
      {/* Lead Details Modal */}
      <LeadActions lead={selectedLead} onClose={() => setSelectedLead(null)} />
    </SafeAreaView>
  );
}