...
const AssignedLeadCard = ({ leadId }: { leadId: string }) => {
  ...
  if (loading) {
    return (
      <View className="bg-white rounded-2xl p-4 mb-3 border border-gray-100 shadow-sm items-center justify-center h-24">
  ...
  return (
    <View className="bg-white rounded-2xl p-4 mb-3 border border-gray-100 shadow-sm flex-row items-center justify-between">
      <View className="flex-1">
        <Text className="text-lg font-bold text-gray-900 mb-1">{lead.farmer_name || "Unknown Farmer"}</Text>
        <Text className="text-sm text-gray-500 mb-2">{lead.phone_number}</Text>
      ...
      <View className="items-end justify-center">
        <View className="bg-blue-50 px-3 py-1 rounded-full border border-blue-100 mb-2">
           <Text className="text-blue-700 text-xs font-bold uppercase tracking-wider">{lead.status}</Text>
        </View>
...
export default function Telecallers() {
  ...
  const handleViewDetails = async (telecaller: Telecaller) => {
    ...
    setAssignedLeadIds([]);
    try {
      // Fetch massive assignments list
      const res = await apiRequest("/manager/tele-assignments");
      const data = await res.json();
      if (!res.ok) throw new Error("Failed to load assignments");
      
      // Filter by the selected telecaller id in javascript
  ...
  const renderItem = ({ item }: { item: Telecaller }) => {
    return (
      <View className="bg-white rounded-3xl border border-gray-100 shadow-sm p-5 mb-5">
        {/* Card Header Layer */}
        <View className="flex-row justify-between items-start mb-6">
          <View className="flex-row items-center flex-1">
            ...
            <View className="relative mr-4">
              <View className="w-14 h-14 bg-blue-100 rounded-full items-center justify-center border-2 border-white shadow-sm">
                <Text className="text-xl font-bold text-blue-700">{getInitials(item.name)}</Text>
              </View>
              {/* Online Dot */}
            ...
            <View className="flex-1">
              ...
              <View className="flex-row items-center gap-1.5 mt-0.5">
                ...
                <Text className="text-sm font-medium text-gray-500">{item.phone}</Text>
              </View>
            </View>
          </View>
          ...
          {/* Right Tag/Status */}
          <View className="items-end justify-center">
            <Text className="text-sm font-extrabold text-blue-600 tracking-tight">ACTIVE</Text>
            <Text className="text-[10px] font-bold text-gray-400 mt-1 uppercase tracking-wider">{item.role}</Text>
          </View>
        </View>
        ...
        <View className="flex-row items-center justify-between px-2 mb-6">
          <View className="items-center flex-1">
            ...
            <Text className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-1">Username</Text>
          </View>
          <View className="w-[1px] h-8 bg-gray-200" />
          <View className="items-center flex-1">
            <Text className="text-base font-bold text-gray-900" numberOfLines={1}>{item.email || "N/A"}</Text>
            <Text className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-1">Email</Text>
          </View>
        </View>

        {/* View Details Button */}
        <TouchableOpacity 
          className="bg-blue-50 py-3.5 rounded-xl items-center justify-center border border-blue-100"
          onPress={() => handleViewDetails(item)}
        >
          <Text className="text-blue-700 font-bold uppercase tracking-wider text-sm">View Details</Text>
        </TouchableOpacity>
      </View>
    );
  };
    ...
    <SafeAreaView edges={["top"]} className="flex-1 bg-[#f8fafc]">
      ...
      <View className="bg-white border-b border-gray-100 pb-4">
        <View className="flex-row items-center justify-between px-6 py-4">
          <View className="flex-row items-center gap-3">
            ...
            <Text className="text-2xl font-extrabold text-gray-900">Telecallers</Text>
          </View>
        </View>
          ...
          isLoading ? null : (
            <View className="mb-6">
              ...
              <View className="flex-row justify-between mb-2 gap-4">
                <View className="flex-1 bg-indigo-50/50 border border-indigo-100/50 rounded-2xl p-4 shadow-sm">
                  <Text className="text-xs font-bold text-indigo-400 uppercase tracking-wider mb-2">Total Calls Today</Text>
                  <Text className="text-base font-extrabold text-gray-400">Coming Soon</Text>
                </View>

                <View className="flex-1 bg-emerald-50/50 border border-emerald-100/50 rounded-2xl p-4 shadow-sm">
        ...
        ListEmptyComponent={
          isLoading ? (
            ...
            <View className="items-center justify-center py-20 bg-white rounded-3xl border border-gray-100 shadow-sm mt-4">
              ...
              <Text className="text-gray-500 font-medium text-lg mt-3">No telecallers match your search.</Text>
            </View>
          )
        }
      />
      ...
      <Modal visible={!!selectedTelecaller} animationType="slide" onRequestClose={() => setSelectedTelecaller(null)}>
        <SafeAreaView className="flex-1 bg-gray-50">
           {/* Header */}
          <View className="flex-row items-center justify-between px-4 py-4 bg-white shadow-sm border-b border-gray-100">
            <View className="flex-row items-center gap-2">
              <TouchableOpacity onPress={() => setSelectedTelecaller(null)} className="p-2">
                <MaterialIcons name="arrow-back" size={24} color="#111827" />
              </TouchableOpacity>
              <View>
                <Text className="text-lg font-bold text-gray-900">{selectedTelecaller?.name}'s Leads</Text>
              </View>
            </View>
          </View>
          ...
          {assignmentsLoading ? (
            <View className="flex-1 items-center justify-center">
               <ActivityIndicator size="large" color="#2563eb" />
               <Text className="text-gray-500 mt-4">Fetching assignments...</Text>
            </View>
          ) : assignedLeadIds.length === 0 ? (
             <View className="flex-1 items-center justify-center px-4">
               <MaterialIcons name="list-alt" size={48} color="#cbd5e1" style={{ marginBottom: 10 }} />
               <Text className="text-gray-400 font-medium text-lg">No leads assigned to this telecaller.</Text>
             </View>
          ) : (
            <FlatList 
              ...
              windowSize={5}
              renderItem={({ item }) => <AssignedLeadCard leadId={item} />}
            />
          )}
