...
export default function Profile() {
  ...
  if (loading) {
    return (
      <SafeAreaView className="flex-1 bg-gray-50 dark:bg-slate-900 items-center justify-center">
  ...
  const InfoCard = ({ icon, label, value }: { icon: keyof typeof MaterialIcons.glyphMap; label: string; value: string }) => (
    <View className="flex-row items-center border-b border-gray-50 dark:border-slate-700/50 py-4 px-2">
      <View className="w-10 h-10 bg-blue-50 dark:bg-slate-700/50 rounded-full items-center justify-center mr-4">
  ...
  return (
    <SafeAreaView edges={['top']} className="flex-1 bg-gray-50 dark:bg-slate-900">
      ...
      <View className="px-6 py-4 bg-white dark:bg-slate-800 border-b border-gray-200 dark:border-slate-700">
        <Text className="text-2xl font-extrabold text-gray-900 dark:text-slate-50">Ground Manager Profile</Text>
      </View>
      ...
      <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 100 }}>
        ...
        <View className="bg-white dark:bg-slate-800 rounded-3xl p-6 shadow-sm border border-gray-100 dark:border-slate-700 items-center mt-2 mb-6">
          <View className="w-24 h-24 bg-blue-100 dark:bg-blue-900/30 rounded-full items-center justify-center border-4 border-white dark:border-slate-800 shadow-sm mb-4">
            <Text className="text-4xl font-extrabold text-blue-600 dark:text-blue-400">{initials}</Text>
          </View>
          <Text className="text-2xl font-bold text-gray-900 dark:text-slate-50 mb-1">{user?.name || 'Unknown Manager'}</Text>
          <View className="bg-blue-50 dark:bg-blue-900/30 px-3 py-1.5 rounded-full border border-blue-100 dark:border-blue-900/50 mt-2">
            <Text className="text-blue-700 dark:text-blue-300 text-xs font-bold uppercase tracking-wider">{user?.role || 'Field Manager'}</Text>
          </View>
        </View>
