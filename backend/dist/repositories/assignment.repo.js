...
class AssignmentRepository {
    ...
    async getAllTeleAssignments() {
        const res = await db_1.pool.query(`SELECT * FROM tele_assignments ORDER BY id DESC`);
        return res.rows;
    }
    async getTeleAssignmentById(id) {
        const res = await db_1.pool.query(`SELECT * FROM tele_assignments WHERE id = $1`, [id]);
        return res.rows[0];
    }
