...
exports.requireFieldExecAccess = exports.requireTelecallerAccess = void 0;
exports.requireRole = requireRole;
exports.requireLeadOwnership = requireLeadOwnership;
exports.requireCampaignOwnership = requireCampaignOwnership;
exports.requireAppropriateLeadStatus = requireAppropriateLeadStatus;
 ...
 */
async function requireLeadOwnership(req, res, next) {
    if (!req.user) {
    ...
    try {
        const result = await db_1.pool.query('SELECT assigned_to FROM leads WHERE id = $1', [leadId]);
...
async function requireCampaignOwnership(req, res, next) {
    ...
    try {
        const result = await db_1.pool.query('SELECT created_by FROM campaigns WHERE id = $1', [campaignId]);
        if (result.rows.length === 0) {
            return res.status(404).json({
                error: 'Not Found',
 ...
 */
async function requireAppropriateLeadStatus(req, res, next) {
    if (!req.user) {
        return res.status(401).json({ error: 'Unauthorized' });
    }
    const leadId = req.params.leadId || req.body.leadId;
    ...
    try {
        const result = await db_1.pool.query('SELECT status FROM leads WHERE id = $1', [leadId]);
        if (result.rows.length === 0) {
            return res.status(404).json({
                error: 'Not Found',
                message: 'Lead not found'
 ...
 */
exports.requireTelecallerAccess = [
 ...
 */
exports.requireFieldExecAccess = [
...
async function requireActiveUser(req, res, next) {
    ...
    try {
        const result = await db_1.pool.query('SELECT is_active FROM users WHERE id = $1', [req.user.id]);
