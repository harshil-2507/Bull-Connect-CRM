...
router.post('/users', async (req, res) => {
    try {
        ...
        const result = await db_1.pool.query(`INSERT INTO users (name, username, password_hash, role, phone, email, is_active)
       VALUES ($1, $2, $3, $4, $5, $6, true)
       RETURNING id, name, username, role, phone, email, is_active, created_at`, [name, username, passwordHash, role, phone, email]);
        const newUser = result.rows[0];
        // Create audit log
        await (0, audit_service_1.auditFromRequest)(req, 'USER', newUser.id, 'CREATE', {
            username,
            role,
        });
        res.status(201).json({
 ...
 */
router.post('/campaigns/:campaignId/assign-leads', async (req, res) => {
    try {
        ...
        const telecallerResult = await db_1.pool.query('SELECT id, role, is_active FROM users WHERE id = $1', [telecallerId]);
        ...
        const assignedLeads = await (0, transactions_1.withTransaction)(async (tx) => {
            // Get unassigned leads
            const leadsResult = await tx.query(`SELECT id FROM leads
           WHERE campaign_id = $1
             AND status = 'NEW'
             AND assigned_to IS NULL
           ORDER BY created_at ASC
           LIMIT $2
           FOR UPDATE`, [campaignId, count]);
           ...
           SET assigned_to = $1,
               status = 'ASSIGNED',
               updated_at = NOW()
           WHERE id = ANY($2)`, [telecallerId, leadIds]);
            // Create assignment records
            for (const leadId of leadIds) {
                await tx.query(`INSERT INTO assignments (lead_id, user_id, assigned_by, is_active)
             VALUES ($1, $2, $3, true)`, [leadId, telecallerId, req.user.id]);
                // Create audit log for each assignment
                await tx.query(`INSERT INTO audit_logs (user_id, entity_type, entity_id, action, metadata)
             VALUES ($1, $2, $3, $4, $5)`, [
                    req.user.id,
                    'LEAD',
                    leadId,
...
router.get('/reports/campaign/:campaignId', async (req, res) => {
    try {
        ...
        const campaignResult = await db_1.pool.query('SELECT * FROM campaigns WHERE id = $1', [campaignId]);
        ...
        // Get lead statistics
        const leadsStatsResult = await db_1.pool.query(`SELECT 
          status,
          COUNT(*) as count,
        ...
        const telecallerStatsResult = await db_1.pool.query(`SELECT 
          u.id,
          u.name,
        ...
        ORDER BY field_requests DESC, interested_calls DESC`, [campaignId]);
        // Get conversion funnel
        const funnelResult = await db_1.pool.query(`SELECT 
        ...
        const totalLeads = Object.values(leadsByStatus).reduce((sum, stat) => sum + stat.count, 0);
        res.json({
            campaign: {
                id: campaign.id,
                name: campaign.name,
...
router.get('/audit-logs', async (req, res) => {
    try {
        const { entityType, action, userId, startDate, endDate, limit = 100, } = req.query;
        const filters = {};
        if (entityType)
        ...
        const logs = await (0, audit_service_1.getRecentAuditLogs)(filters, Number(limit));
        res.json({
            logs,
            count: logs.length,
            filters,
...
router.get('/audit-stats', async (req, res) => {
    try {
        ...
        const stats = await (0, audit_service_1.getAuditStatistics)(startDate ? new Date(startDate) : undefined, endDate ? new Date(endDate) : undefined);
        res.json(stats);
    }
    catch (error) {
...
router.patch('/users/:userId/status', async (req, res) => {
    try {
       ...
       WHERE id = $2
       RETURNING id, name, username, role, is_active`, [isActive, userId]);
        ...
        // Create audit log
        await (0, audit_service_1.auditFromRequest)(req, 'USER', userId, 'UPDATE', {
...
router.get('/dashboard', async (req, res) => {
    try {
        // Get user counts
        const userStatsResult = await db_1.pool.query(`
      SELECT 
        role,
    ...
    `);
        ...
        // Get campaign stats
        const campaignStatsResult = await db_1.pool.query(`
      SELECT 
        COUNT(*) as total,
    ...
    `);
        ...
        // Get lead stats
        const leadStatsResult = await db_1.pool.query(`
      SELECT 
        status,
        COUNT(*) as count
      FROM leads
    ...
    `);
        ...
        const todayActivityResult = await db_1.pool.query(`
      SELECT 
